...
public interface IndividuAutocompleteRepository extends JpaRepository<Individu, Integer> {
    @Query(value =
        "SELECT i.id AS individu_id, i.nom, i.prenoms AS prenom, i.date_naissance, i.contact_mada AS contact, " +
        "p.numero_pass AS passeport_numero, p.date_delivrance, p.date_expiration, " +
        "i.nom_jeune_fille, i.id_situation_familiale AS situation_familiale_id, i.nationalite, i.profession, i.adresse_mada " +
        "FROM dev.individu i " +
